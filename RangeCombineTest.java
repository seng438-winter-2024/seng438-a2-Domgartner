...
public class RangeCombineTest {
	...
	public void combineValidrangeAndNullRange(){
		...
		try {
			...
			assertEquals("If first argument is null, and the second is a valid range, the second range should be returned.",rangeA, res);
		}
		catch(Exception e) {
	...
	public void combineNullrangeAndValidRange(){
		...
		try {
			...
			assertEquals("If first argument is a valid range, and the second is null, the second range should be returned.",rangeB, res);		}
		catch(Exception e) {
			fail("Combining these values should not throw an error.");
		}
	}
    ...
    public void combineNonIdenticalEmptyRanges() {
        ...
        assertEquals("New range must return arange that subsumes both empty ranges.", emptyRange, Range.combine(rangeA, rangeB));
    }
    ...
    public void combineValidRangeAndSubsetRange() {
        ...
        assertEquals("Combining two valid ranges where one is a subset should return the superset.", expected, Range.combine(rangeA, rangeB));
    }
    ...
    public void combineTouchingValidRanges() {
		...
		try {
			Range res =  Range.combine(rangeA, rangeB);
	        assertEquals("Combining two touching valid ranges should result in an expected superset range", expected, res);
    ...
    public void combineOverlappingValidRanges() {
		...
		try {
			Range res =  Range.combine(rangeA, rangeB);
	        assertEquals("Combining two overlapping valid ranges should result in an expected superset range", expected, res);
    ...
    public void combineIdenticalValidRanges() {
        ...
        assertEquals("Combining two identical valid ranges should result in a third identical range", expected, Range.combine(rangeA, rangeB));
    }
